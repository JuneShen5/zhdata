		...
		<div class="col-sm-7 <c:if test="${att.inputLength=='2'}"> col-sm-3</c:if>">
            <c:choose>
                <c:when test="${att.inputType=='input'}">
                    <input type="text" name="${att.code}" class="form-control hasNoSpace" placeholder="${att.remarks}"
                        <c:if test="${att.isRequire==1}" >required</c:if>>
                </c:when>
                <c:when test="${att.inputType=='textarea'}">
                    <textarea type="text" name="${att.code}" class="form-control" placeholder="${att.remarks}"
                        <c:if test="${att.isRequire==1}" >required</c:if> 
                        style="min-height: 115px;"></textarea>
                </c:when>
                <c:when test="${att.inputType=='select'}">
                    <select name="${att.code}" class="select-chosen" <c:if test="${att.isRequire==1}" >required</c:if>>
                        <option value=""></option>
                ...
                <c:when test="${att.inputType=='dateselect'}">
                    <input type="text" name="${att.code}" class="form-control datepicker"
                        <c:if test="${att.isRequire==1}" >required</c:if> value="" readonly="readonly">
                </c:when>
                <c:when test="${att.inputType=='radiobox'}">
                    <c:forEach var="obj" items="${fns:getDictList(att.inputValue)}">
                        <label class="radio-inline i-checks">
                            <input type="radio" value="${obj.value}" name="${att.code}-radio">  ${obj.label}</label>
                    </c:forEach>
                    <input type="text" stype="checkbox" name="${att.code}" class="checkboxInput hide"
                    <c:if test="${att.isRequire==1}" >required</c:if>>
                </c:when>
                ...
                <c:when test="${att.inputType=='upload'}">
                    <div class="col-sm-5 detail-hide edit-show" style="width: calc(100% - 190px); padding-left: 0;">
                        <input type="text" name="${att.code}" class="form-control detail-hide"
